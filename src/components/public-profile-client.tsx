          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
            ...
            <div className="flex-1 text-center sm:text-left pt-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                  <h1 className="text-3xl font-black font-heading text-white mb-2 tracking-tight">
                    {user.name || dict.profile.anonymous}
                  </h1>
                  {user.bio && (
                    <p className="text-dark-muted text-sm max-w-lg leading-relaxed">{user.bio}</p>
                  )}
                  <div className="flex items-center gap-4 mt-4 text-xs text-neon-cyan/70 font-medium justify-center sm:justify-start bg-neon-cyan/5 w-fit px-3 py-1.5 rounded-full border border-neon-cyan/10">
                ...
                {!isSelf && (
                  <button
                    ...
                    disabled={isPending}
                    className={`px-8 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all duration-500 transform hover:scale-105 ${
                      isFollowing 
                        ? "bg-dark-surface/50 border border-dark-border text-dark-text hover:border-neon-pink/50 hover:text-neon-pink" 
                        : "bg-white text-dark-bg hover:shadow-[0_0_20px_rgba(255,255,255,0.4)]"
                    }`}
                  >
                    {isFollowing ? (
                      <><UserMinus className="w-4 h-4" />{dict.profile.unfollow}</>
                    ) : (
                      <><UserPlus className="w-4 h-4" />{dict.profile.follow}</>
                    )}
                  </button>
                )}
              </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {statCards.map((s) => (
                ...
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${s.color} bg-dark-bg border border-white/5 shadow-inner`}>
